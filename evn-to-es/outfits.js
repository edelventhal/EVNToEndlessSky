...
var getCategory = function( data )
{
    if ( data.ModType === 1 )
    {
        return flagGunStrings[ data.Flags.charAt( data.Flags.length - 1 ) ];
    }
    return categoryStrings[ data.ModType ];
};

var outfitToString = function( id, name, data, description )
{
    var str = "outfit \"" + name + "\"\n";
    str += "\tcategory \"" + getCategory( data ) + "\"\n";
    str += "\tcost " + data.Cost + "\n";
    ...
    //so, we don't want to add a tag even if it has a value of 0
    if ( data.Mass !== 0 )
    {
...
var evndata = JSON.parse( fs.readFileSync( "evndata.json", "utf8" ) );
...
for ( outfitIndex = 0; outfitIndex < outfits.length; outfitIndex++ )
{
    ...
    if ( !outfitLookup[ outfit.data.ShortName ] ) //TODO - we need to handle the weird duplicates somehow
    {
        ...
        result += outfitToString( outfit.id, outfit.data.ShortName, outfit.data, desc ) + "\n";
        
        outfitLookup[ outfit.data.ShortName ] = true;
    }
}
...
for ( outfitIndex = 0; outfitIndex < outfits.length; outfitIndex++ )
{
    result += "\t\"" + outfits[ outfitIndex ].data.ShortName + "\"\n";
}

//write them to a text file
fs.writeFileSync( "evnoutfits.txt", result );