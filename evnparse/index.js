...
var Converter =
{
    fileParsesRemaining: 0,
    results: {},
    templates: JSON.parse( fs.readFileSync( process.argv[1] + "/templates.json", "utf8" ) ),
    includeRawData: false,
    parsingTemplate: false,
    
    convertFolder: function( dataFolder )
    {
        var files = [];
    ...
    parseResource: function( resourceData, resourceArray, type )
    {
    ...
    interpretData: function( resource, type )
    {
        resource.data = {};
    ...
    getNextAvailableKey: function( data, desiredKey )
    {
        var key = desiredKey;
    ...
    interpretTemplateData: function( resource, type )
    {
        resource.templates = [];
    ...
    bytesToString: function( bytes, trim )
    {
        if ( trim )
        {
            var startIndex = 0;
    ...
    //65535 = -1, 65534 = -2
    bytesToNumber: function( x, signed )
    {
        var val = 0;
        var i;
    ...
    bytesToHexString: function( arr, prettifier )
    {
        var result = "";
        var z;
    ...
    arrayFromBytes: function( bytes, byteCountObj )
    {
    ...
    portData: function()
    {
        if ( this.parsingTemplate )
        {
            ...
            fs.writeFileSync( process.argv[1] + "/templates.json", JSON.stringify( data, null, "\t" ) );
        }
        else
        {
            fs.writeFileSync( process.argv[1] + "/evndata.json", JSON.stringify( this.results, null, "\t" ) );
        }
    }
};
...
{
    console.log( "node EVNToEndlessSky <path/to/evnData>" );
}
else
{
    ...
    if ( process.argv[3] === "-r" )
    {
        Converter.includeRawData = true;
    }
    else if ( process.argv[3] === "-t" )
    {
        Converter.parsingTemplate = true;
    }
    
    Converter.convertFolder( dataFolder );
}